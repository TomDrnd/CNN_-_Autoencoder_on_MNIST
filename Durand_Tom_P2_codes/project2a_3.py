...
def init_weights_bias_rms4(filter_shape, d_type):
    ...
    r_values = np.zeros(filter_shape, dtype=d_type)
    return theano.shared(w_values,borrow=True), theano.shared(b_values, borrow=True), theano.shared(r_values,borrow=True)

def init_weights_bias_rms2(filter_shape, d_type):
...
def sgd(cost, params, rms):
    grads = T.grad(cost=cost, wrt=params)
    updates = []
    for r, g, p in zip(rms, grads[0::2], params[0::2]):
        updates.append([r, rho*r +(1-rho)*((g + decay*p)**2)])
        updates.append([p, p-(learning_rate/np.sqrt(epsilon+rho*r+(1-rho)*((g + decay*p)**2)))*(g + decay*p)])
...
num_filters_h1 = 15
num_filters_h2 = 20
w1, b1, r1 = init_weights_bias_rms4((num_filters_h1, 1, 9, 9), X.dtype)
w2, b2, r2 = init_weights_bias_rms4((num_filters_h2, num_filters_h1, 5, 5), X.dtype)
w3, b3, r3 = init_weights_bias_rms2((num_filters_h2*3*3, 100), X.dtype)
w4, b4, r4 = init_weights_bias_rms2((100, 10), X.dtype)
...
#pylab.plot(range(noIters), a, '-g', label='RMSProp')
#pylab.savefig('project2a_3_test_accuracy.png')
#pickle.dump(acc, file('accuracy_mnist.pickle', 'w'))

#acc = pickle.load(file('cost_mnist.pickle'))
#pylab.plot(range(noIters), cost, '-g', label='RMSProp')
#pylab.savefig('project2a_3_cost.png')
...
pylab.savefig('project2a_3_filters_h0.png')
...
pylab.savefig('project2a_3_filters_h1.png')
...
for j in ind:
    ...
    pylab.savefig('project2a_3_input_'+str(j)+'.png')
    ...
    pylab.savefig('project2a_3_input_'+str(j)+'_features_h0.png')
    ...
    pylab.savefig('project2a_3_input_'+str(j)+'_pooled_features_h0.png')
    ...
    pylab.savefig('project2a_3_input_'+str(j)+'_features_h1.png')
    ...
    pylab.savefig('project2a_3_input_'+str(j)+'_pooled_features_h1.png')
